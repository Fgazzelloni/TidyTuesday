...
Exploratory analysis on Gross investments shows increase in investments in the private sector as the most flourishing category within the last 70 years, followed by basic, S&L and social investments in infrastructures. Digital infrastructure and transports are still below 200 000 millions $.
...
Before continuing with other visualizations of investment trends, let's check the other data sets to see the differences and then decide if to make a unified dataframe to use for comparison of the gross investments, the chained investments with the implicit price deflactors (IPDs).

The first 4 variables are in common within the three datasets, the next step is to check whether there are differences within the first 4 variables.
...
"Office buildings, NAICS 518 and 519" is the only category in "investment" df which doesn't appear in "chain_investment" df.
